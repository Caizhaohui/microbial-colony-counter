...
> **用少量已有真值的参考盘，为本批次自动搜索一套参数 θ\*，再把 θ\* 应用到其余平板。**
...
记参考盘图像为 **I**，人工真值菌落数为 **N**，在参数 **θ** 下的自动计数记为 **count(I, θ)**。

**单盘目标（直观描述）**：在候选参数中找到一套 **θ\***，使自动计数尽量接近人工真值，即让 `|count(I, θ) − N|` 尽可能小。

```text
单盘：  θ* = argmin_θ  | count(I, θ) − N |
```

$$
\theta^{\ast}=\arg\min_{\theta}\ \lvert \mathrm{count}(I,\theta)-N\rvert
$$
...
**多盘目标（直观描述）**：对 K 块参考盘同时评估，使各盘误差的 **平均值** 最小，得到一套共享参数 **θ\***。

```text
多盘：  θ* = argmin_θ  (1/K) × Σ_k  Loss( count(I_k, θ), N_k, 可选点集_k )
```

$$
\theta^{\ast}=\arg\min_{\theta}\ \frac{1}{K}\sum_{k=1}^{K}\mathrm{Loss}\bigl(\mathrm{count}(I_{k},\theta),\,N_{k},\,\text{points}_{k}\bigr)
$$

- 每块盘：图片 + 真值 **N<sub>k</sub>**（点选可选）  
- 优化目标：各盘误差的 **平均值** 最小  
- 输出：一套共享参数 θ\*，以及 **每盘** 的「预测 vs 真值」报告  
...
- `backend/core/batch.py` — 多图套用同一 θ\*  
