...
记参考盘图像为 \(I\)，人工真值菌落数为 \(N\)，自动计数为 \(\mathrm{count}(I, \theta)\)。  
**单盘目标**近似为：

\[
\theta^{*} = \arg\min_{\theta}\ \bigl|\mathrm{count}(I, \theta) - N\bigr|
\]
...
\mathrm{Loss}\bigl(\mathrm{count}(I_k, \theta),\ N_k,\ \text{可选点集}_k\bigr)
\]
